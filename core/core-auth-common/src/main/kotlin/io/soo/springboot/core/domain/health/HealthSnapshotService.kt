...
class HealthSnapshotService(
    ...
    fun publicSummary(req: HealthRequest): Map<String, Any> {
    ...
    fun adminDetails(req: HealthRequest): Map<String, Any> {
    ...
    private fun checkMonitoring(req: HealthRequest): Map<String, Any?> {
    ...
    private fun publicLinks(req: HealthRequest): Map<String, Any> {
        return linkedMapOf(
            "docs" to linkedMapOf(
    ...
    private fun adminLinks(req: HealthRequest): Map<String, Any> {
        return linkedMapOf(
            "docs" to linkedMapOf(
                "swagger" to resolveLink(healthLinksProperties.docs.swagger, req),
                "docs" to resolveLink(healthLinksProperties.docs.docs, req),
    ...
    private fun resolveLink(template: String, req: HealthRequest): String {
        if (template.isBlank()) return template
        if (template.startsWith("http://") || template.startsWith("https://")) return template

        val scheme = forwardedFirst(req.forwardedProto) ?: req.scheme
        val hostHeader = forwardedFirst(req.forwardedHost)
            ?: req.hostHeader
            ?: req.serverName
    ...
    private fun buildBaseUrl(req: HealthRequest, scheme: String, hostHeader: String): String {
        if (hostHeader.contains(":")) return "$scheme://$hostHeader"

        val forwardedPort = forwardedFirst(req.forwardedPort)?.toIntOrNull()
