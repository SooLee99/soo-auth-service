...
class AdminUserBlockService(
    ...
    fun listBlocked(pageable: Pageable): Page<User> {
        return userRepository.blocked(pageable)
    }

    @Transactional(readOnly = true)
    fun listDeleted(pageable: Pageable): Page<User> {
        return userRepository.deleted(pageable)
    }

    @Transactional(readOnly = true)
    fun listLogs(targetUserId: Long, pageable: Pageable): Page<UserStatusAuditLog> {
